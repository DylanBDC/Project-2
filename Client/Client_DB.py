...
#check to see if the client is running on a RPI
try:
...
try:
    ...
    for i in range(50): # 50 iterations
        ...
        # The LED will turn on and off following the LED state
        if Led_state:
            window[f'-LED{0}-'].update('\u2B24')
                    
        else:
            window[f'-LED{0}-'].update('\u25EF')
        # in each loop the LED will change state   
    ...
    s.close()
    exit(1)
except OSError: # trouble connecting
    print("Cant connect")
    s.close()
    exit(1)
except:
